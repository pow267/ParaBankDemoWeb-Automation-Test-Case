...
public class JdbcBookstoreDao extends JdbcDaoSupport implements AdminDao {
    ...
    @Override
    public void cleanDB() {
        // Not yet implemented for Bookstore mock
    }
    ...
    public synchronized void initializeDB() {
        ...
        try {
            con  = getConnection();
            if (con != null) {
                ScriptUtils.executeSqlScript(con, Objects.requireNonNull(CREATE_RESOURCE));
                ScriptUtils.executeSqlScript(con, Objects.requireNonNull(INSERT_RESOURCE));
            }
        } finally {
            if (con != null) {
