...
public class JdbcAccountDao extends NamedParameterJdbcDaoSupport implements AccountDao {
    private static class AccountMapper implements RowMapper<Account> {
        @Override
        public Account mapRow(@NonNull final ResultSet rs, final int rowNum) throws SQLException {
     ...
     */
    @Override
    public int createAccount(@NonNull final Account account) {
        final String SQL =
            "INSERT INTO Account (id, customer_id, type, balance) VALUES (:id, :customerId, :intType, :balance)";

        final int id = Objects.requireNonNull(sequenceDao).getNextId("Account");
        account.setId(id);
    ...
    public Account getAccount(final int id) {
        ...
        final Account account = Objects.requireNonNull(getJdbcTemplate()).queryForObject(SQL, new AccountMapper(), id);
    ...
    public List<Account> getAccountsForCustomerId(final int customerId) {
        ...
        final List<Account> accounts = Objects.requireNonNull(getJdbcTemplate()).query(SQL, new AccountMapper(), customerId);
