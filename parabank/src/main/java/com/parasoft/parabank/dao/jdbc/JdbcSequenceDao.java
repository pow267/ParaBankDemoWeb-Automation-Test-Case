...
public class JdbcSequenceDao extends JdbcDaoSupport {
    ...
    public int getCurrentId(final String name) {
        final Number number = Objects.requireNonNull(getJdbcTemplate()).queryForObject("SELECT next_id FROM Sequence WHERE name = ?",
            Integer.class, name);
    ...
    public int getNextId(final String name) {
        ...
        // int nextId = getJdbcTemplate().queryForInt("SELECT next_id FROM
        // Sequence WHERE name = ?", name);
        Objects.requireNonNull(getJdbcTemplate()).update("UPDATE Sequence SET next_id = ? WHERE name = ?", nextId + OFFSET, name);
        return nextId;
    }
