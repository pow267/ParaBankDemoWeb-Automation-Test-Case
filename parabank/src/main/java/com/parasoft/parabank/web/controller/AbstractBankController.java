...
public abstract class AbstractBankController implements IAccessModeControllerAware, ApplicationContextAware {
    ...
    protected <T> T getCommand(final Model model) {
    ...
    protected <T> T prepCommand(final Model model) throws Exception {
        @SuppressWarnings("unchecked")
        final T command = (T) getCommandClass().getDeclaredConstructor().newInstance();
        return prepCommand(model, command);
    }

    protected <T> T prepCommand(final Model model, final T command) {
        model.addAttribute(Objects.requireNonNull(getCommandName()), command);
        return command;
    }

    protected ModelAndView prepForm(final Model model) throws Exception {
        final Object command = getCommandClass().getDeclaredConstructor().newInstance();
        return prepForm(model, command);
    }

    protected ModelAndView prepForm(final Model model, final Object command) {
        final ModelAndView mav = new ModelAndView(Objects.requireNonNull(getFormView()));
        if (model == null) {
            mav.addObject(Objects.requireNonNull(getCommandName()), command);
        } else {
            model.addAttribute(Objects.requireNonNull(getCommandName()), command);
    ...
    /** {@inheritDoc} */
    @Override
    public void setApplicationContext(@NonNull final ApplicationContext aApplicationContext) throws BeansException {
