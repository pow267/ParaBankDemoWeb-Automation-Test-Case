...
public class JmsLoanProvider implements LoanProvider {
    ...
    LoanResponse processResponse(final Message message) {
        if (message != null) {
            try {
                ...
                final Object obj = MarshalUtil.unmarshal(Objects.requireNonNull(unmarshaller), Objects.requireNonNull(text));
            ...
            log.error(
                "Did not receive response message within timeout period of " + Objects.requireNonNull(jmsTemplate).getReceiveTimeout() + " ms");
    ...
    public LoanResponse requestLoan(final LoanRequest loanRequest) {
        Objects.requireNonNull(jmsTemplate).send(Objects.requireNonNull(requestDesinationName), session -> {
            final String xml = MarshalUtil.marshal(Objects.requireNonNull(marshaller), Objects.requireNonNull(loanRequest));
            return Objects.requireNonNull(session.createTextMessage(xml));
        });
        final Message message = jmsTemplate.receive(Objects.requireNonNull(responseDestinationName));
