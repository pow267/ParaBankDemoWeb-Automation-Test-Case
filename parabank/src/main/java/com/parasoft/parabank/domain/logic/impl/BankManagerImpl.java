...
public class BankManagerImpl implements BankManager {
    ...
    public List<Position> buyPosition(final int customerId, final int accountId, final String name, final String symbol,
        ...
        final int positionId = Objects.requireNonNull(positionDao).createPosition(Objects.requireNonNull(position));
        log.info("Created position with id = " + positionId + " with " + shares + " shares");

        final Customer customer = Objects.requireNonNull(getCustomer(customerId));
     ...
     */
    @Override
    public int createAccount(@NonNull final Account account, final int fromAccountId) {
        final int id = Objects.requireNonNull(accountDao).createAccount(account);

        transfer(fromAccountId, id, new BigDecimal(Objects.requireNonNull(adminDao).getParameter(AdminParameters.MINIMUM_BALANCE)));
     ...
     */
    @Override
    public int createCustomer(@NonNull final Customer customer) {
        final int id = Objects.requireNonNull(customerDao).createCustomer(customer);
        log.info("Created customer with id = " + id);
        final Account account = new Account();
        account.setCustomerId(id);
        account.setType(AccountType.CHECKING);
        account.setBalance(new BigDecimal(Objects.requireNonNull(adminDao).getParameter(AdminParameters.INITIAL_BALANCE)));
        Objects.requireNonNull(accountDao).createAccount(account);
    ...
    private Transaction createTransaction(final Account account, final TransactionType type, final Date date,
        ...
        transaction.setAccountId(Objects.requireNonNull(account).getId());
    ...
    public boolean deletePosition(@NonNull final Position position) {
        return Objects.requireNonNull(positionDao).deletePosition(position);
    }
    ...
    public Transaction deposit(final int accountId, final BigDecimal amount, final String description) {
        final Account account = Objects.requireNonNull(accountDao).getAccount(accountId);

        Objects.requireNonNull(account).credit(amount);
        ...
        Objects.requireNonNull(transactionDao).createTransaction(Objects.requireNonNull(transaction));
    ...
    public Account getAccount(final int id) {
        return Objects.requireNonNull(accountDao).getAccount(id);
    }
     ...
     */
    @Override
    public List<Account> getAccountsForCustomer(@NonNull final Customer customer) {
        return Objects.requireNonNull(accountDao).getAccountsForCustomerId(customer.getId());
    }
    ...
    public Customer getCustomer(final int id) {
        return Objects.requireNonNull(customerDao).getCustomer(id);
    }
    ...
    public Customer getCustomer(final String ssn) {
        return Objects.requireNonNull(customerDao).getCustomer(ssn);
    }
    ...
    public Customer getCustomer(final String username, final String password) {
        return Objects.requireNonNull(customerDao).getCustomer(username, password);
    }
    ...
    public Position getPosition(final int positionId) {
        return Objects.requireNonNull(positionDao).getPosition(positionId);
    }
    ...
    public List<HistoryPoint> getPositionHistory(final int positionId, final Date startDate, final Date endDate) {
        return Objects.requireNonNull(positionDao).getPositionHistory(positionId, startDate, endDate);
    }
     ...
     */
    @Override
    public List<Position> getPositionsForCustomer(@NonNull final Customer customer) {
        return Objects.requireNonNull(positionDao).getPositionsForCustomerId(customer.getId());
    }
    ...
    public Transaction getTransaction(final int id) {
        return Objects.requireNonNull(transactionDao).getTransaction(id);
    }
     ...
     */
    @Override
    public List<Transaction> getTransactionsForAccount(@NonNull final Account account) {
        return Objects.requireNonNull(transactionDao).getTransactionsForAccount(account.getId());
    }
    ...
    public List<Transaction> getTransactionsForAccount(final int accountId, final TransactionCriteria criteria) {
        return Objects.requireNonNull(transactionDao).getTransactionsForAccount(accountId, criteria);
    }
    ...
    public LoanResponse requestLoan(final int customerId, final BigDecimal amount, final BigDecimal downPayment,
        ...
        final List<Account> accounts = Objects.requireNonNull(accountDao).getAccountsForCustomerId(customerId);
        ...
        final LoanResponse loanResponse = Objects.requireNonNull(loanProvider).requestLoan(loanRequest);

        if (Objects.requireNonNull(loanResponse).isApproved()) {
            ...
            final int accountId = Objects.requireNonNull(accountDao).createAccount(loanAccount);
    ...
    public List<Position> sellPosition(final int customerId, final int accountId, final int positionId,
        final int shares, final BigDecimal pricePerShare) {
        final Position position = Objects.requireNonNull(positionDao).getPosition(positionId);
        
        if (position == null) {
            log.error("Position not found for id: " + positionId);
            final Customer customer = Objects.requireNonNull(getCustomer(customerId));
            return getPositionsForCustomer(customer);
        }
    ...
    public void updateCustomer(@NonNull final Customer customer) {
        Objects.requireNonNull(customerDao).updateCustomer(customer);
    }
    ...
    public boolean updatePosition(@NonNull final Position position) {
        return Objects.requireNonNull(positionDao).updatePosition(position);
    }
    ...
    public void withdraw(final int accountId, final BigDecimal amount, final String description) {
        ...
        Objects.requireNonNull(account).debit(amount);
        accountDao.updateAccount(account);
