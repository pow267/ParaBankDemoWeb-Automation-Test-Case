...
public class LoanRequestTestConfig implements InitializingBean {
    ...
    public void afterPropertiesSet() throws IOException {
        ...
        if (isLoanRequest()) {
            final BigDecimal bd = getAmountBigDecimal().setScale(2, RoundingMode.HALF_EVEN);
            setAmount(bd.toPlainString());
            if (getDownPayment() == null || getDownPayment().isEmpty()) {
                final BigDecimal downPayment = bd.divide(new BigDecimal(10), 2, RoundingMode.HALF_EVEN);
