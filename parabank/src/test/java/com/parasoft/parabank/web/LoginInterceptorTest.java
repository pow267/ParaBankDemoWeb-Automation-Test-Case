...
public class LoginInterceptorTest extends AbstractParaBankTest {
    ...
    private ModelAndView assertLoginForm(final LoginInterceptor interceptor) throws Exception {
        try {
            assertFalse(Objects.requireNonNull(interceptor).preHandle(Objects.requireNonNull(request), Objects.requireNonNull(response), new Object()));
            ...
            assertEquals(Constants.LOGINFORM, Objects.requireNonNull(mav).getViewName());
            return mav;
        }
    ...
    public void testLoginInterceptor() throws Exception {
        ...
        assertEquals(SERVLET_PATH, Objects.requireNonNull(Objects.requireNonNull(mav).getModel()).get("loginForwardAction"));

        Objects.requireNonNull(request).setQueryString(QUERY_STRING);
        mav = assertLoginForm(interceptor);
        assertEquals(SERVLET_PATH + "?" + QUERY_STRING, Objects.requireNonNull(Objects.requireNonNull(mav).getModel()).get("loginForwardAction"));
        ...
        assertTrue(interceptor.preHandle(Objects.requireNonNull(request), Objects.requireNonNull(response), new Object()));
    }
}
