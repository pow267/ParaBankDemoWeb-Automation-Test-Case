...
public class IndexControllerTest extends AbstractControllerTest<IndexController> {
    ...
    public void testDatabaseUninitialized() throws Exception {
        Objects.requireNonNull(getJdbcTemplate()).execute("DROP TABLE News");
        ModelAndView mav =
        ...
        assertNotNull(mav);
        assertEquals("/initializeDB.htm", ((RedirectView) Objects.requireNonNull(mav.getView())).getUrl());
        setPath("/initializeDB.htm");
        setFormName("none");
        mav = processGetRequest(registerSession(new MockHttpServletRequest()), new MockHttpServletResponse());
        assertNotNull(mav);
        assertEquals("/index.htm", ((RedirectView) Objects.requireNonNull(Objects.requireNonNull(mav).getView())).getUrl());
    ...
    public void testHandleGetRequest() throws Exception {
        ...
        assertEquals(Constants.INDEX, Objects.requireNonNull(mav).getViewName());

        final Date date = (Date) getModelValue(mav, "date");
        assertEquals(today.toString(), Objects.requireNonNull(date).toString());

        final List<News> news = (List<News>) getModelValue(mav, Constants.NEWS);
        assertEquals(3, Objects.requireNonNull(news).size());
    }
}
