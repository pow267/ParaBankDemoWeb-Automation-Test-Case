...
public class JdbcPositionDaoTest extends AbstractParaBankDataSourceTest {
    ...
    public void onSetUp() throws Exception {
        super.onSetUp();
        if (Objects.requireNonNull(stockDataInserter).getDataCount() < 1) {
    ...
    public void testCreatePosition() {
        ...
        final int positionId = Objects.requireNonNull(positionDao).createPosition(setUpPosition);
    ...
    public void testGetPosition() {
        Position position = Objects.requireNonNull(positionDao).getPosition(12345);
        assertEquals(12345, Objects.requireNonNull(position).getPositionId());
        ...
        // 1351.1200 issue
        assertEquals(new BigDecimal("23.53").floatValue(), Objects.requireNonNull(position.getPurchasePrice()).floatValue(), 0.0001f);
    ...
    public void testGetPositionHistory() {
        ...
        final List<HistoryPoint> history = Objects.requireNonNull(positionDao).getPositionHistory(12345, startDate, endDate);
        int expected = 11;
        if (Objects.requireNonNull(history).size() == 22) { // TODO figure out why when this test class
    ...
    public void testGetPositionsForCustomerId() {
        List<Position> positions = Objects.requireNonNull(positionDao).getPositionsForCustomerId(12212);
        assertEquals("wrong number of positions?", 3, Objects.requireNonNull(positions).size());

        positions = positionDao.getPositionsForCustomerId(-1);
        assertEquals("expected no positions for invalid id", 0, Objects.requireNonNull(positions).size());
    }
