...
public abstract class AbstractValidatorTest extends AbstractParaBankTest {
    ...
    public void testSupports() {
        assertTrue(getValidator().supports(Objects.requireNonNull(beanClass)));
        assertFalse(getValidator().supports(Object.class));
    }

    @Test
    public void testValidate() throws Exception {
        BeanWrapper wrapper = new BeanWrapperImpl(Objects.requireNonNull(beanClass));
    ...
    static final void assertValidate(Validator validator, Object target, String[] requiredFields) throws Exception {
        ...
        for (Field field : fields) {
            ...
            assertNotNull("expected error for empty field: " + field.getName(), errors.getFieldError(Objects.requireNonNull(field.getName())));
            ...
            assertNull("expected error for non-empty field: " + field.getName(), errors.getFieldError(Objects.requireNonNull(field.getName())));
            assertEquals(--expectedErrorCount, errors.getErrorCount());
        }
    }
}
